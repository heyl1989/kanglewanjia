...
import cn.v1.kanglewanjia.ui.EvaluateActivity;
...
public class SingleCallActivity extends BaseCallActivity implements Handler.Callback {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_ASK_MULTIPLE_PERMISSIONS) {
            ...
            if (null != data && data.getBooleanExtra("endSession", false)) {
...
//        }
        if (callAction.equals(RongCallAction.ACTION_INCOMING_CALL)) {
            ...
            orderType = (String) SPUtil.get(context, Common.INQUIRY_ORDER_TYPE, "");
    ...
    public void onCallOutgoing(RongCallSession callSession, SurfaceView localVideo) {
        ...
        if (callSession.getMediaType().equals(RongCallCommon.CallMediaType.VIDEO)) {
            ...
            imageView.setImageResource(R.drawable.bg_call_out);
            mLPreviewContainer.addView(imageView);
//            mLPreviewContainer.addView(localVideo);
    ...
    public void onCallConnected(RongCallSession callSession, SurfaceView localVideo) {
        ...
        if (callSession.getMediaType().equals(RongCallCommon.CallMediaType.AUDIO)) {
            findViewById(R.id.rc_voip_call_minimize).setVisibility(View.GONE);
            FrameLayout btnLayout = (FrameLayout) inflater.inflate(R.layout.rc_voip_call_bottom_connected_button_layout, null);
            btnLayout.findViewById(R.id.rc_voip_call_mute).setVisibility(View.GONE);
    ...
    public void onRemoteUserJoined(final String userId, RongCallCommon.CallMediaType mediaType, SurfaceView remoteVideo) {
        ...
        if (mediaType.equals(RongCallCommon.CallMediaType.VIDEO)) {
            ...
            mLPreviewContainer.setBackgroundColor(Color.parseColor("#000000"));
            mUserInfoContainer.setVisibility(View.GONE);
            mUserInfoContainer.findViewById(R.id.rc_voip_call_minimize).setVisibility(View.GONE);
            mButtonContainer.setVisibility(View.VISIBLE);
            FrameLayout btnLayout = (FrameLayout) inflater.inflate(R.layout.rc_voip_call_bottom_connected_button_layout, null);
            btnLayout.findViewById(R.id.rc_voip_call_hang_up).requestFocus();
            btnLayout.findViewById(R.id.rc_voip_call_mute).setVisibility(View.GONE);
            btnLayout.findViewById(R.id.rc_voip_handfree).setVisibility(View.GONE);
            btnLayout.findViewById(R.id.rc_voip_camera).setVisibility(View.GONE);
            mButtonContainer.removeAllViews();
...
//            mButtonContainer.setVisibility(View.GONE);
    ...
    public void onHangupBtnClick(View view) {
        Bitmap bmp = FastBlurUtility.getBlurBackgroundDrawer(SingleCallActivity.this);
        imgBg.setImageBitmap(bmp);
        Intent intent = new Intent(this, DialogActivity.class);
        intent.putExtra("from", "singleCall");
...
//                break;
        ...
        if (TextUtils.equals("CallOutgoing", callState)) {
            cancelOrder();
...
//        showShortToast(reason.getValue() + "");
    }
...
//    }
    ...
    private <T> void bindObservable(final Observable<T> observable, final Action1<T> nextAction, final Action1<Throwable> errorAction) {
    ...
    private CompositeSubscription getmCompositeSubscription() {
    ...
    private void cancelOrder() {
        bindObservable(new AppClient().cancelOrder((String) SPUtil.get(context, Common.INQUIRY_ORDER_TYPE, ""), orderId), new Action1<BaseData>() {
            @Override
    ...
    private void completeOrder() {
        bindObservable(new AppClient().completeOrder((String) SPUtil.get(context, Common.INQUIRY_ORDER_TYPE, ""), orderId, getTime() + ""), new Action1<BaseData>() {
            @Override
            public void call(BaseData data) {
                if (TextUtils.equals("0000", data.getCode())) {
